...
kubectl exec -it "${POD_NAME}" -n "${NAMESPACE}" -- bash -c "PATH=\"\$PATH:/root/go/bin/\" dnspyre -s \"${KUBE_DNS_IP}\" @/app-services-fqdn.txt -c ${CONCURRENCY} --duration=${TEST_DURATION} --log-requests --log-requests-path \"${LOG_FILE}\" --probability=1" || { echo "ERROR: dnspyre command failed inside the pod."; exit 1; }
